...
async fn desktop_styles() -> impl IntoResponse {
    let css = tokio::fs::read_to_string("static/styles.css")
        .await
        .unwrap();
...
async fn mobile_styles() -> impl IntoResponse {
    let css = tokio::fs::read_to_string("static/mobile.css")
        .await
        .unwrap();
    ([(header::CONTENT_TYPE, "text/css")], css)
}
...
async fn main_js() -> impl IntoResponse {
    let js = tokio::fs::read_to_string("static/main.js")
        .await
        .unwrap();
...
async fn reset_styles() -> impl IntoResponse {
    let css = tokio::fs::read_to_string("static/reset.css")
        .await
        .unwrap();
    ([(header::CONTENT_TYPE, "text/css")], css)
}
